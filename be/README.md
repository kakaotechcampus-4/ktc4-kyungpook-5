...
- AI 연동 계약 — `infra/agent_client.py`가 호출할 AI 내부 API의 엔드포인트명과 요청·응답 스키마
- 인증·인가 방식과 세션(또는 토큰) 관리 방식
- `Approval`, `AgentRun` 모델을 `models/events.py`에 포함할지 별도 파일로 분리할지, 그리고 이 기록을 BE가 직접 쌓을지 AI가 보고한 내용을 저장할지
