...
```jsonc
{
  "data": {
    ...
    "warnings": [
      { "code": "SCHEDULE_GAP", "message": "입금 마감(3/12)과 리허설(3/20) 사이가 8일 비어 있습니다.", "suggestion": "중간 점검 단계 넣기" }
    ],
    "excludedSteps": [
...
| 필드 | 타입 | 설명 |
| --- | --- | --- |
| `stepIds` | string[] | 새 순서대로 나열한 전체 단계 ID |

```jsonc
// 요청
{ "stepIds": ["stp_02", "stp_01", "stp_03", "stp_04", "stp_05", "stp_06", "stp_07"] }
```
...
{
  "data": [
    { "id": "stp_02", "stepOrder": 10, "name": "세부사항 조정" },
    { "id": "stp_01", "stepOrder": 20, "name": "사전 수요 조사" }
  ],
  "meta": null
}
```

> `stepOrder`는 서버가 10·20·30… 으로 다시 부여합니다.
...
| `expectedHeadcount` | int | 🔸 컬럼 추가 필요 |
...
| `completedCount` / `totalActionCount` | int | `steps.completed` / Action 총수 |
...
> ⚠️ `steps.completed`가 0이고 Action도 0개인 단계(MANUAL)는 `completedCount == totalActionCount`가 되어 항상 완료로 계산됩니다.
> **사람이 직접 완료 처리하는 수단이 필요합니다.** → `POST /steps/{stepId}:complete` 를 추가하거나 `steps`에 `completed_at` 컬럼을 두는 방안. **BE 확인 필요.**
...
| `approvedAt` | datetime \| null | 처리 시각 |
| `approvedBy` | object \| null | `{ id, name, role }` — 처리자 |
  ...
  "data": [
    {
      ...
      "approvedAt": null, "approvedBy": null, "denyReason": null,
      "canApprove": true
    },
    {
      ...
      "approvedAt": "2026-03-08T08:02:00Z",
      "approvedBy": { "id": "mbr_01", "name": "박수겸", "role": "MANAGER" },
...
승인합니다. `status`가 `APPROVED`로 바뀌고 `approved_by` / `approved_at`이 기록됩니다.
...
```jsonc
{
  "data": {
    "id": "act_21",
    "status": "APPROVED",
    "approvedAt": "2026-03-10T05:31:00Z",
    "approvedBy": { "id": "mbr_01", "name": "박수겸", "role": "MANAGER" }
  },
  "meta": null
}
...
**parseError 형식 (JSONB)** — BE와 합의 필요

| 필드 | 타입 | 설명 |
| --- | --- | --- |
| `reason` | enum | `COLUMN_MISMATCH` / `IMAGE_NOT_READABLE` / `UNSUPPORTED_FORMAT` |
| `detail` | string | 사용자에게 보여줄 한 문장 |
| `expectedColumns` | string[] | 열 불일치일 때 |
| `actualColumns` | string[] | |
| `suggestions` | array | `[{ "action": "MAP_COLUMNS", "label": "열 직접 맞추기" }]` |
...
```jsonc
{
  "data": {
    ...
    "parseError": {
      ...
      "suggestions": [
        { "action": "MAP_COLUMNS", "label": "열 직접 맞추기" },
        { "action": "DOWNLOAD_TEMPLATE", "label": "양식 받아서 다시 만들기" }
      ]
    },
...
`parseError.suggestions`의 `MAP_COLUMNS`에 대응하는 엔드포인트입니다.
...
```jsonc
{
  "data": {
    ...
    "sources": [
      { "recordId": "rec_03", "fileName": "2025 하반기 장부.xlsx", "location": "회비 수납 시트" },
      { "recordId": "rec_05", "fileName": "2025 봄 MT 결과보고.hwp", "location": "예산 항목" }
    ],
...
무엇이 왜 필요한지는 별도 문서로 정리했습니다 → **`운영해_BE_스키마_검토요청.md`**
